...
- **Idle overlay:** The normal handle collapses after three seconds into a visually `4dp` Signal Mint stripe while retaining a transparent `48dp` touch target. Stripe and drawer meet the physical right edge with zero visual gap; the revealed handle uses `14dp` corners on its exposed left edge and a square attached right edge. Swipe inward or tap the stripe to reveal the Material Symbols `open_in_full` handle; tap the revealed handle to open the drawer.
