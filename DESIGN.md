...
- **Idle overlay:** The normal handle collapses after three seconds into a visually `4dp` Signal Mint stripe while retaining a transparent `48dp` touch target. Stripe and drawer meet the physical right edge with zero visual gap; the revealed handle retains all four `14dp` corners inside its flush edge bounds. Swipe inward or tap the stripe to reveal the Material Symbols handle; tap the revealed handle to open the drawer.
