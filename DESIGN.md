...
- **Idle overlay:** The normal handle collapses after three seconds into a visually `4dp` Signal Mint stripe while retaining a transparent `48dp` touch target. Stripe, revealed handle, and drawer all meet the physical right edge with zero visual gap. Swipe inward or tap the stripe to reveal the Material Symbols handle; tap the revealed handle to open the drawer.
